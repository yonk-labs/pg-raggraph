...
- **63 passing tests** (21 unit + 42 integration/E2E)
...
- Edge case tests that found 4 real bugs during development
...
### 2. Tests exist but aren't all run ⚠️
The "core" test suite is 63 tests. But there are also `test_real_llm.py`, `test_user_journey.py`, `test_retrieval_comparison.py`, `test_graph_wins.py` that need a live LLM to run, and they're excluded from default `pytest`. In CI, we'd need a way to run them. Right now they're aspirational.
...
### 4. The README example showed a non-existent feature ⚠️
Original README had `print(result.answer)` but `result.answer` is always empty. We fixed it in docs but the fundamental issue remains: **pg-raggraph doesn't generate answers**. It returns chunks with provenance. Users expecting ChatGPT-style answers will be surprised. There's no LLM answer synthesis layer.
...
### 1. Ingestion fails on edge cases in production ❌
During the 909-doc pg_agents ingest:
- **Deadlocks** when two parallel docs try to resolve the same entity at the same time (handled via try/except, but docs are lost)
- **Pydantic validation errors** when the LLM returns `weight: null` for a relationship (one doc lost)
- **~5% of files failed** — 909 made it out of ~963 eligible files

We have no retry logic for these failures. The ingested counts silently drop without the user knowing.

### 2. No incremental update story ❌
Content hash dedup prevents re-ingesting identical files, but if a file CHANGES, the old document stays in the graph alongside the new version. There's no "update in place" — just "ignore unchanged" and "insert new." Stale entities accumulate. No way to clean them up except `pgrg delete -n namespace` and start over.

### 3. No entity update/deletion API ❌
`GraphRAG.delete(namespace)` drops everything in a namespace. There's no way to delete a specific document without wiping the namespace. No way to manually fix a bad entity extraction. No way to merge two entities the resolver missed.

### 4. Graph visualization only exists in the demo UI ❌
The `/graph` endpoint returns nodes/edges, but there's no SDK method to export the graph for visualization in other tools. No way to export as DOT, GraphML, or JSON from the CLI.

### 5. "Smart mode" is a misleading name ❌
It's not AI, it's not adaptive, it's a threshold-based router. "Smart" sets the wrong expectations. Better names: `auto`, `routed`, `tiered`, `adaptive`. Too late to rename without breaking things, but worth calling out.

### 6. Dev KB extraction prompt is only half-tuned ❌
The `dev` prompt lists good entity types (person, service, library, file, commit, incident, adr) but the LLM only sometimes follows them. On pg_agents we got 377 services but also 3,366 `concept` entities — which means most of the extraction defaulted back to the generic "concept" bucket. The prompt needs few-shot examples.

### 7. No answer generation ❌
This is the biggest product gap. Every RAG tool in the wild generates an answer from the retrieved chunks. pg-raggraph returns `QueryResult.chunks` and sets `answer=""`. Users have to do the final LLM call themselves. For a "dead simple" library, this is not dead simple — users will bolt on their own answer generation and get it wrong.

### 8. MCP server advertised but not built ❌
`PROJECT.md` and marketing materials mention an MCP server for Claude Code and other agents. It's not built. The skeleton exists in comments but no actual MCP server module.

### 9. No LangChain / LlamaIndex adapters ❌
Also advertised, also not built. We have the core retrieval — wrapping it in a `BaseRetriever` is 30 lines — but we haven't done it.

### 10. No real CI/CD ❌
There's no `.github/workflows/`, no automated testing on PR, no badge. The 63 tests exist but nothing enforces they keep passing. Right now I have to run `uv run pytest` manually to know the state.

### 11. The Docker setup is dev-only ❌
`docker-compose.yml` spins up PostgreSQL on port 5434 with postgres/postgres credentials. There's no production deployment story. No Helm chart, no Terraform, no "here's how to run this on AWS." Users who want to deploy this are on their own.

### 12. Schema migration is CREATE IF NOT EXISTS only ❌
We can create the v1 schema but have no story for v2. When we need to add a column, drop an index, or change a type, existing installs break. No Alembic, no versioned migrations, no upgrade path.
...
1. **Answer generation layer** — take retrieved chunks, send to LLM, return actual answer. This is the single biggest UX win and probably 100 LOC. Without it, the tool is a retrieval API, not a RAG system.

2. **Fix the ingestion failures** — add retry logic for deadlocks, handle `weight=null` in the Pydantic model, log/surface failures. Shouldn't silently drop 5% of documents.

3. **Entity dedup pass** — after ingestion, run a cleanup to merge entities like `OpenAI` / `Open AI` that slipped past resolution. Current resolver catches most but not all.

4. **CI/CD** — GitHub Actions running the 63 core tests on every PR. Without this, tests rot.

5. **Answer synthesis + MCP server** — these together make it usable by Claude Code as a tool. `pgrg mcp-serve` is the path to "plug into your agent."

6. **Schema migration framework** — Alembic or similar. Without this, v0.1.0 → v0.2.0 breaks every existing install.

7. **Fix the SEC 47% ceiling** — investigate the chunking issue, add table-aware extraction, see if we can hit 70%+ on gold-standard multi-doc financial questions.

8. **Cut `global` and `hybrid` from the public API** — keep them internal. Users should see 3 modes: `naive`, `smart` (default), and maybe `local` for explicit graph expansion.

---

## Final Grade

If I had to grade this with a letter:

- **Architecture:** A- (sound, portable, novel graph-boost approach)
- **Implementation:** B (63 tests pass, but rough edges and some gaps)
- **Documentation:** B+ (dense, honest, multiple guides)
- **Production readiness:** C (works on real data, but missing deploy/migration/retry/MCP)
...
**Overall: B/B+.** An impressive proof of concept with a real accuracy win. **Not ready to ship as v1.0**, but the core engine is worth building on. With 2-3 focused sprints on the "rough" list above, this could be a credible open-source release.

The thing that would move it from B+ to A is **answer generation + MCP server + CI**. Those three changes turn a retrieval library into a usable GraphRAG system.
