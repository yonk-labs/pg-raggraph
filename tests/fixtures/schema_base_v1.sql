-- FROZEN FIXTURE — DO NOT UPDATE.
--
-- This is src/pg_raggraph/sql/schema.sql exactly as it existed immediately
-- before migration 001_embedded_content.sql shipped (git 58d8c1d~1): the
-- oldest base schema the forward-only migration chain upgrades from.
-- tests/integration/test_migration_parity.py bootstraps a database from this
-- file, runs migrations 001..head through the production runner, and asserts
-- table/column/index parity with a fresh schema.sql bootstrap. Updating this
-- file to a newer schema would silently shrink the upgrade path under test.
--
