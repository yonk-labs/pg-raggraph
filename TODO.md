...
| T-07 | **Chunkshop direction** | ✅ done | Option C chosen: chunkshop stays a sibling library. pg-raggraph does not import chunkshop. Chunkshop is experimentation + benchmarking surface; pg-raggraph is product. Winners are ported (with lineage annotation) into pg-raggraph under new `chunk_strategy` values. See `docs/chunkshop-integration.md`. |
| T-08 | **Chunkshop integration** | ✅ done | Shipped as a policy + doc, not a runtime dep. `docs/chunkshop-integration.md` documents the relationship, port policy, and the four use cases. Lineage annotation added to `src/pg_raggraph/chunking.py`. No library code depends on chunkshop. MS + pg-src bakeoff work (T-20/T-21) runs through the bakeoff's existing chunkshop path dep without touching pg-raggraph. |
